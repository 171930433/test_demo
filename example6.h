...
public:
  ...
  key_from_key(
      ...
      const KeyExtractor2 &key2_ = KeyExtractor2()) : key1(key1_), key2(key2_)
  {
  }
...
struct car_model
{
  ...
  car_model(
      const std::string &model_, const car_manufacturer *manufacturer_, int price_) : model(model_), manufacturer(manufacturer_), price(price_)
  {
  }
